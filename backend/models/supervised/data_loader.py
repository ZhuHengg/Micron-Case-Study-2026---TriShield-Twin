...
def split_data(df: pd.DataFrame, test_size: float = 0.2, val_size: float = 0.2, random_state: int = 42):
    ...
    # Verify defect rate is preserved across all three sets
    print(f"\nDefect rate check:")
    print(f"  Original: {df['is_defective'].mean():.4f}")
    print(f"  Train:    {df_train['is_defective'].mean():.4f}")
    print(f"  Val:      {df_val['is_defective'].mean():.4f}")
    print(f"  Test:     {df_test['is_defective'].mean():.4f}")
