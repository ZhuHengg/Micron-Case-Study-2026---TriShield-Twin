...
export default function Header({ engine, activeTab }) {
  ...
  return (
    <header className="h-[80px] bg-bg-base flex items-center justify-between px-8 border-b border-border z-10 relative">
      <div>
        <h2 className="font-sans font-bold text-[22px] text-text-primary uppercase tracking-tight">
          {title}
        </h2>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="hidden md:flex items-center gap-2 mr-4 bg-bg-100 px-3 py-1.5 rounded-full border border-border">
          ...
          <span className="font-sans text-[11px] font-bold text-text-secondary uppercase">
            {isRunning ? 'Unit Stream Active' : 'Stream Paused'}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 bg-bg-50 p-1.5 rounded-2xl border border-border shadow-sm">
          <button 
            onClick={resetParams}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-sans font-bold text-text-secondary hover:text-text-primary hover:bg-bg-200 transition-colors uppercase tracking-wide"
          >
            <RefreshCw size={14} />
            Reset Specs
          </button>
          ...
          <button 
            ...
            className={clsx(
              "flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-sans font-bold transition-all border uppercase tracking-wide",
              isRunning 
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-[12px] font-sans font-bold text-[#ef4444] bg-[rgba(239,68,68,0.1)] border border-[rgba(239,68,68,0.3)] hover:bg-[rgba(239,68,68,0.2)] transition-colors shadow-glow-block ml-1 uppercase tracking-wide"
          >
            <Flame size={14} />
            Yield Excursion
          </button>
        ...
        <div className="ml-4 flex items-center gap-3">
          ...
          <div className="w-10 h-10 rounded-full bg-gradient-signature p-[2px]">
            <div className="w-full h-full rounded-full bg-bg-base border-2 border-transparent flex items-center justify-center">
              <span className="font-sans font-bold text-[13px] text-white">YI</span>
            </div>
          </div>
        </div>
